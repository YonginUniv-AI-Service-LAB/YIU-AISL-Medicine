...
export default function SharePage({ navigation }: any) {
  ...
  const [myGuestMessages, setMyGuestMessages] = useState<any[]>([
    { id: '1', date: '24.02.19 10:00', senderEmail: 'tester@abc.com', message: '반갑습니다! 좋은 하루 되세요.' }
  ]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        {/* 검색창 */}
        <TouchableOpacity style={styles.searchBox} onPress={() => setIsAddPopupVisible(true)}>
        ...
        <View style={styles.guestRow}>
          ...
          {isFriendView && (
            <>
              <TouchableOpacity style={styles.editIconWrapper} onPress={() => setIsGuestBookVisible(true)}>
                <Image 
              ...
              >
                <Image source={require('../../assets/images/share/calender.png')} style={styles.calendarIcon} />
              </TouchableOpacity>
            </>
          )}
          <TouchableOpacity style={styles.friendListIconWrapper} onPress={() => setIsPopupVisible(true)}>
            <Image 
              source={isPopupVisible 
        ...
        <View style={{ paddingHorizontal: 20, marginTop: 20 }}>
          {isFriendView ? (
            friendGuestMessages.length > 0 ? (
              friendGuestMessages.map((item) => <GuestBookCard key={item.id} data={item} />)
            ) : (
              <Text style={{ textAlign: 'center', color: '#D9D9D9', marginTop: 40 }}>
                친구에게 첫 방명록을 남겨보세요.
              </Text>
            )
          ) : (
            myGuestMessages.length > 0 ? (
              myGuestMessages.map((item) => (
                <GuestBookCard 
                  ...
                  data={item} 
                  showDelete={true} 
                  onDelete={(id) => setMyGuestMessages(myGuestMessages.filter(m => m.id !== id))}
                />
              ))
            ) : (
              <Text style={{ textAlign: 'center', color: '#D9D9D9', marginTop: 40 }}>
      ...
      <FrendPopup
        ...
        onDelete={(email) => setFriendList(friendList.filter(e => e !== email))}
