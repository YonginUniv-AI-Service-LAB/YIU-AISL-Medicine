...
export default function SignUpScreen() {
  ...
  const handleEmailVerify = () => {
    if (!email || !email.includes('@')) {
      return Alert.alert('알림', '올바른 이메일 형식을 입력해주세요.');
    }
  ...
  // 2. 인증번호 체크
  const onChangeCertNum = (v: string) => {
    setCertNum(v);
    if (v === '123456') { // 예시 번호
  ...
  const handleIdCheck = () => {
    if (!userId || !userId.includes('@')) {
      return Alert.alert('알림', '아이디로 사용할 올바른 이메일을 입력해주세요.');
    }
  ...
  const handleSignUpComplete = () => {
    Alert.alert(
      ...
      [
        { 
          text: '확인', 
          onPress: () => navigation.navigate('LoginPage') // Navigation.tsx의 이름과 일치
        }
      ]
    );
  };
  ...
  const isFormValid = 
    ...
    pw.length >= 8 && 
    pw === pwCheck && 
    agrees.age && agrees.tos && agrees.privacy;
        ...
        <View style={styles.contentWrapper}>
          ...
          <View style={styles.row}>
            <View style={styles.emailInputContainer}>
              ...
              />
              {emailSent && <Image source={CHECK_GREEN_ICON} style={styles.insideCheckIcon} resizeMode="contain" />}
            </View>
            <TouchableOpacity 
          ...
          <View style={styles.codeWrapper}>
            <TextInput 
              ...
              editable={emailSent && !certNumValid}
            />
          ...
          <TextInput style={styles.fullInput} placeholder="홍길동" placeholderTextColor="#D9D9D9" value={name} onChangeText={setName} />
          ...
          <View style={styles.row}>
            <View style={styles.emailInputContainer}>
              <TextInput 
                ...
                placeholderTextColor="#D9D9D9"
                value={userId}
                onChangeText={(v) => {setUserId(v); setIdChecked(false);}}
                editable={!idChecked}
              />
              {idChecked && <Image source={CHECK_GREEN_ICON} style={styles.insideCheckIcon} resizeMode="contain" />}
            </View>
            <TouchableOpacity style={styles.smallBtn} onPress={handleIdCheck} disabled={idChecked}>
              <Text style={styles.smallBtnText}>중복확인</Text>
            </TouchableOpacity>
          </View>

          <Text style={styles.inputLabel}>비밀번호</Text>
          <TextInput style={styles.fullInput} placeholder="영문, 숫자, 특수문자 포함 8자 이상" placeholderTextColor="#D9D9D9" secureTextEntry value={pw} onChangeText={setPw} />

          <Text style={[styles.inputLabel, pwCheck.length > 0 && pw !== pwCheck && { color: '#F84545' }]}>
